...
public class AppointmentRepository extends BaseMongoRepository<Appointment> {
  ...
  public Appointment create(String patientId, String slotId, String visitReason) {
    ...
    Slot slot = slotRepository.update(slotId, criteria, new Update().set("status", SlotStatus.BOOKED));
  ...
  public void update(String id, AppointmentStatus status, String visitReason) {
    ...
    if (status.equals(AppointmentStatus.REJECTED) || status.equals(AppointmentStatus.CANCELLED)) {
